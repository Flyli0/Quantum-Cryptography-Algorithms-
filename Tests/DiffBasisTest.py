...
for rrr in range(trials):
    ...
    # TEST
    print(f"-=(|)=- -=(|)=- -=(|)=- -=(|)=-      ⛤key {rrr}⛤      -=(|)=- -=(|)=- -=(|)=- -=(|)=- -=(|)=- -=(|)=-")
    print(f'number of quants: {qubits_number}')
    print(f'number of matched bases: {len(final_key)+checking_len}')
    print(f"IN PERCENTS, match: {((len(final_key)+checking_len)/qubits_number)*100}%")
    result.append(((len(final_key)+checking_len)/qubits_number))
res = 0
for i in result:
    res+=i
print(f"༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻   FINAL RESULT: {(res/trials)*100} %   ༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻༺❀༻")